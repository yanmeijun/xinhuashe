...
serviceLogic.searchByKwd = async(ctx) => {
    ...
    if(body.keyword){//关键字
        query = {$or: [{serviceName: new RegExp(body.keyword)},{summary:new RegExp(body.keyword)}]}
    }
    if (body.category) {//分类
        query.serviceID = new RegExp('^' + body.category)
    }
    if (body.serviceType) {//类型
        query.serviceType = Number(body.serviceType)
    }
    if (body.price) {//价格
        query.price = body.price == "0"? 0 : { $gt: Number(body.price) }
    }
    if (body.city) {//范围
        const cityReg = '^' + body.city.substring(0, 2);
        query.city = {$in: ['all', new RegExp(cityReg)]}
    }
    try {
