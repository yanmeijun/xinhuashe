var encrysrc = localStorage.getItem("encryStr");//记录号（接口5详情参数）
var encryname = localStorage.getItem("encryname");
$(function () {
    $('#dialogMask,#dialog').show();
    getPurchaseIllegalDetail()//页面初始化执行
})

// 点击返回按钮
$('#back').on('click', function () {
    var url = "/credit?page=governmentProcurementInquiry&randomKey=" + randomKey + "&userID=" + userID.trim() + "&clientID=" + clientID.trim() + "&cityID=" + cityID.trim() + "&local_x=" + local_x + "&localFrom=" + localFrom +  "&local_y=" + local_y;
...
function getPurchaseIllegalDetail() {
    var parameters = {
        randomKey: randomKey,
        userID: userID,
        clientID: clientID,
    ...
    $.ajax({
        async: "true",
        type: "post",
        url: "/credit/getPurchaseIllegalDetails",
        ...
        success: function (data) {
            ...
            if(data.responseBody.data.indexOf("202")>0){
                masktime("网络超时，请刷新页面");
                return
            }
...
function companyRedList(data) {
    ...
    if (data.hmd != null && data.hmd.length != 0) {
        $.each(data.hmd, function (index, val) {
            html += '<div class="tab-content blacklist ListLineBox" style="display: none">' +
                ...
                '<div class="text-list-div clearfix">' +
                '<label>纳税人名称：</label>' +
                '<span class="color-fa6e08">' + val["纳税人名称"] + '</span>' +
                '</div>' +
                '<div class="text-list-div clearfix">' +
                '<label>评价年度：</label>' +
                '<span>' + val["评价年度"] + '</span>' +
