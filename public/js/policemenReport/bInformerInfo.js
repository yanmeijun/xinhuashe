...
$('#verificationCode').click(function () {
    ...
    $.ajax({
        async: true,
        url: "/policemenReport/verificationCode",
        type: 'post',
...
function ajaxData(beReportName, beReportUnit, citySelect, cardType, suspectedNature, specialStatus, suspectedArea, content, reportImg) {
    ...
    $.ajax({
        async: true,
        url: "/policemenReport/report",
        type: "post",
        data: JSON.stringify(parameters),
        contentType: 'application/json',
        success: function (res) {
            if (res.retCode == "000000") {
                sessionStorage.setItem("policemenReport", JSON.stringify(res.responseBody));
                if (res.responseBody.status.indexOf("成功") != -1) {
                    window.location.href = "/policemenReport?page=success&randomKey=" + randomKey + "&userID=" + userID +
                        "&clientID=" + clientID + "&cityID=" + cityID + "&local_x=" + local_x + "&localFrom=" + localFrom + "&local_y=" + local_y;
                    return;
                } else {
                    window.location.href = "/policemenReport?page=failed&randomKey=" + randomKey + "&userID=" + userID +
                        "&clientID=" + clientID + "&cityID=" + cityID + "&local_x=" + local_x + "&localFrom=" + localFrom + "&local_y=" + local_y;
                    return;
                }
            }
...
document.getElementById("goback").onclick = function () {
    window.location.href = "/policemenReport?page=informerInfo&randomKey=" + randomKey + "&userID=" + userID +
        "&clientID=" + clientID + "&cityID=" + cityID + "&local_x=" + local_x + "&localFrom=" + localFrom + "&local_y=" + local_y;
    return;
};
...
$('#area,#suspectedArea').on("click", function () {
    $("#countrySelect").html("");
    $('#titleCon').html("");
    $('#dialogMask,#dialog').show();
    var data = {
    ...
    $.ajax({
        async: true,
        url: "/policemenReport/areasInvolved",
        type: 'post',
        data: JSON.stringify(data),
        contentType: 'application/json',
        success: function (res) {
