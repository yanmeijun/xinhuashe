$('#numResult').css("display", "none");
$(function () {
    $("#veriCodeImage").on("click", function (event) {
        $("#veriCodeImage").attr("src", "/images/yanzm.gif");
        var data = {
            randomKey: randomKey,
            userID: userID,
            clientID: clientID,
            cityID: cityID,
            local_x: local_x ,localFrom:localFrom,
            local_y: local_y
        };
        $.ajax({
            async: true,
            type: 'post',
            data: JSON.stringify(data),
            url: '/unhealthy/verifyCode',
            contentType: 'application/json'
        }).done(function (data) {
            $("#veriCodeImage").attr("src", data);
        });
    });
    $("#veriCodeImage").trigger("click");
});


//点击举报
$("#report").on("click", function (event) {
    var phone = $('#ownphone').val(); //请输入您的手机号码
    var verifycode = $('#verification').val(); //验证码
    var phonecode = $('#messageCode').val();//短信验证码

    if (phone == '') {
        $('#masktime').html("请输入11位数字的手机号码")
        $('#masktime').show();
        setTimeout(function () {
            $('#masktime').hide();
        }, 2000)
        return
    } else if (!(/^1[3|4|5|8|7|9][0-9]\d{8}$/.test(phone))) {
        $('#masktime').html("请输入正确的手机号码")
        $('#masktime').show();
        setTimeout(function () {
            $('#masktime').hide();
        }, 2000)
        return
    }
    if (verifycode == '') {
        $('#masktime').html("请输入验证码")
        $('#masktime').show();
        setTimeout(function () {
            $('#masktime').hide();
        }, 2000)
        return
    }

    if (phonecode == "") {
        $('#masktime').html("短信验证码")
        $('#masktime').show();
        setTimeout(function () {
            $('#masktime').hide();
        }, 2000)
        return
    }
    ;
    $('#dialogMask,#dialog').show();
    var data = {
        randomKey: randomKey,
        userID: userID,
        clientID: clientID,
        cityID: cityID,
        local_x: local_x ,localFrom:localFrom,
        local_y: local_y,
        code: $("#messageCode").val(),
    }

    $.ajax({
        async: true,
        type: 'post',
        data: JSON.stringify(data),
        url: '/unhealthy/findMessageSpeed',
        contentType: 'application/json'
    }).done(function (data) {
        $('#dialogMask,#dialog').hide();
        if (data.retCode == '000000') {
            var datalenthg = data.responseBody.list;
            var htmls = '';
            $.each(datalenthg, function (index, val) {
                htmls += "<div class=\"peccancyQuery userInfor onlyStyle\">" +
                    "<h2 class=\"q-r-tit report-title\"><small>举报时间</small><label>" + val.jbtime + "</label></h2>" +
                    "<hr class=\"hrLine\">" +
                    "<div class=\"tab-content jubao-list\">" +
                    "<div class=\"text-list-div clearfix\">" +
                    "<label>被举报号码：</label>" +
                    "<span>" + val.bjbphone + "</span>" +
                    "</div>" +
                    "<div class=\"text-list-div clearfix\">" +
                    "<label>短信内容：</label>" +
                    "<span>" + val.dxnr + "</span>" +
                    "</div>" +
                    "<div class=\"text-list-div clearfix\">" +
                    "<label>处理状态：</label>" +
                    "<span>" + val.clzt + "</span>" +
                    "</div>" +
                    "<div class=\"text-list-div clearfix\">" +
                    "<label>处理结果：</label>" +
                    "<span>" + val.cljg + "</span>" +
                    "</div>" +
                    "</div>" +
                    "</div>";

            })
            $('#parkingResult').html(datalenthg.length);
            var userInfo = $('#userInfor').html(htmls)
            $('#numResult').append(userInfo);
            $('#numResult').css("display", "block");
        } else {
            $("#tips_text").html(" ");
            $('#mask').hide();
            $("#tips").hide();
            if (data == "Bad request") {
                $("#tips_text").html("请求异常，请稍后")
                $("#tips").show();
                $('#mask').show();
                return;
            } else {
                if (data.responseBody.errorMsg) {
                    $("#tips_text").html(data.responseBody.errorMsg);
                    $("#tips").show();
                    $('#mask').show();
                    return;
                } else {
                    $("#tips_text").html("请求异常，请稍后")
                    $("#tips").show();
                    $('#mask').show();
                    return;
                }
            }

        }

    })
});
//取消提示信息
$('#iKnow').on("click", function () {
    $("#tips").hide();
    $("#mask").hide();
})


var wait = 60;
//点击验证
$("#getCodes").click(function () {
    if ($('#ownphone').val() == "" || $('#verification').val() == "") {
        $('#masktime').html("验证码,电话不能为空");
        $('#masktime').show();
        setTimeout(function () {
            $('#masktime').hide();
        }, 2000)
        return
    } else if (!(/^1[3|4|5|8|7|9][0-9]\d{8}$/.test($('#ownphone').val()))) {
        $('#masktime').html("请输入正确的验证手机号码");
        $('#masktime').show();
        setTimeout(function () {
            $('#masktime').hide();
        }, 2000)
        return
    }
    if (wait == 60) {
        var data = {
            randomKey: randomKey,
            userID: userID,
            clientID: clientID,
            cityID: cityID,
            local_x: local_x ,localFrom:localFrom,
            local_y: local_y,
            code: $("#verification").val(),
            phone: $("#ownphone").val(),
        };
        $.ajax({
            async: true,
            type: 'post',
            data: JSON.stringify(data),
            url: '/unhealthy/getVerifyCode',
            contentType: 'application/json'
        }).done(function (body) {
            if (body.retCode == '000000') {
                $("#tips_text").html("短信已发送到您手机上，如果1分钟内没有收到短信验证码，请点击按钮重新获取，此服务免费。");
                $("#mask").show();
                $("#tips").show();
                time($("#getCodes"));
            } else {
                if (body.responseBody.errorMsg) {
                    $("#tips_text").html(body.responseBody.errorMsg);
                    $("#tips").show();
                    $('#mask').show();
                    return;
                } else {
                    $("#tips_text").html("请求异常，请稍后")
                    $("#tips").show();
                    $('#mask').show();
                    return;
                }
            }
        });
    }

});
//提示信息
function time(but) {
    if (wait == 0) {
        $(but).removeAttr("disabled").removeClass("get_code_wait").addClass("get-code");
        $(but).text("获取校验码");
        wait = 60;
    } else {
        $(but).attr("disabled", true).removeClass("get-code").addClass("get_code_wait");
        $(but).text("重新发送(" + wait + ")");
        wait--;
        setTimeout(function () {
            time(but);
        }, 1000);
    }
}

/*
 点击返回按钮
 */
$('#back').on("click", function () {
    var url = "/unhealthy?randomKey=" + randomKey + "&userID=" + userID +
        "&clientID=" + clientID + "&cityID=" + cityID + "&local_x=" + local_x + "&localFrom=" + localFrom +  "&localFrom=" + localFrom + "&local_y=" + local_y;
    window.location.href = url;
})