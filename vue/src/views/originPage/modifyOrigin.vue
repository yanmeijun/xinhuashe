...
<script type="text/ecmascript-6">
  ...
  export default{
    props: ["isShowModifyPage", "modifyOriginID", "domain", "userID"],
    data(){
      return {
        initData: {},
        originName: "",
    ...
    methods: {
      ...
      getData(){
        axios({
          ...
          url: "/origin/getByID",
          async: true,
          data: {originID: this.modifyOriginID},
          contentType: 'application/json'
        }).then(res=> {
          if (res.data.msg == "success") {
            if (res.data.results.dataCount > 0) {
              const data = res.data.results.dataList[0];
              this.initData = data;
        ...
        axios({
          ...
          if (res.data.code == 401) {
            ...
            if (res.data.msg == "success") {
              if (res.data.results.dataCount > 0) {
                const data = res.data.results.dataList[0];
                if (action == "name" && this.initData.originName != data.originName) {
                  this.ifNameEmpty = true;
                  this.nameErr = "该监控名已存在";
                } else if (action == "url" && this.initData.originUrl != data.originUrl) {
                  this.ifUrlEmpty = true;
