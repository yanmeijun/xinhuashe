const urlParse = require('url'),
    _ = require('underscore'),
    util = require('../../lib/util').util,
    cityDAO = require('../dao/city').cityDAO,
    userLogDAO = require('../dao/userLog').userLogDAO,
    taxOfficeService = require("../logics/taxOffice").taxOfficeService;
const path = "taxOffice/", serviceID = 'KAC0001';
let taxOfficeCon = {};

taxOfficeCon.getCon = (req, res, next) => {
    if (req.method.toUpperCase() === "GET") {
        if (_.isEmpty(req.query.page)) {
            apiCon(req, res, next)
        } else {
            pageCon(req, res, next)
        }
    } else {
        apiCon(req, res, next);
    }
};
//页面跳转方法
const pageCon = (req, res, next) => {
    const renderName = req.query.page;
    console.log("税务事务所查询renderName: " + renderName);
    cityDAO.get(req.query.cityID, (err, cityInfo) => {
        res.render(path + renderName, _.extend(req.query, {cityInfo: cityInfo}));
    })
};
//api跳转方法
const apiCon = (req, res, next) => {
    const pathName = urlParse.parse(req.url).pathname.replace("/", "").split("/")[1];
    console.log("税务事务所查询请求方法名: " + pathName);
    if (!pathName) return;
    eval(pathName + "(req, res, next)");
};
//跳转百度地图
const map = (req, res, next) => {
    const params = {
        searchText: req.query.searchText,
        randomKey: req.query.randomKey,//用户randomKey,用来识别用户的唯一性
        userID: req.query.userID,//用户userID
        clientID: req.query.clientID,//用户设备id
        cityID: req.query.cityID,//城市代号
        local_x: req.query.local_x,//城市经度
        local_y: req.query.local_y//城市纬度
    };
    if (!params.searchText) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}});
    } else {
        const {randomKey, userID, clientID, cityID, local_x, local_y, searchText} = req.query;
        cityDAO.get(cityID, (err, cityInfo) => {
            res.render(path + 'map', {randomKey, cityID, userID, clientID, local_x, local_y, searchText});
        })
    }
};
//全国税务事务所查询
const taxOfficeSearch = (req, res, next) => {
    const params = {
        sessionId: req.body.randomKey,
        taskId: "1",
        serviceId: "taxFirms",
        randomKey: req.body.randomKey,//用户randomKey,用来识别用户的唯一性
        userID: req.body.userID,//用户userID
        clientID: req.body.clientID,//用户设备id
        cityID: req.body.cityID,//城市代号
        local_x: req.body.local_x,//城市经度
        local_y: req.body.local_y//城市纬度
    };
    if (util.isEmptyValue("taxOfficeCon.taxOfficeSearch", params)) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}});
    } else {
        params.articleField02 = req.body.articleField02;//事务所名称
        params.articleField03 = req.body.articleField03;//性质
        params.articleField07 = req.body.articleField07;//所长姓名
        params.articleField09 = req.body.articleField09;//所在地区
        params.cPage = req.body.cPage || '1';//页码
        params.scount = req.body.scount;
        params.randCode = req.body.randCode;
        taxOfficeService.taxOffice(params)
            .then((results) => {
                res.send(results);
                var logData = {
                    localFrom: req.body.localFrom,//项目用户名称
                    clientID: req.body.clientID,//用户设备ID
                    serviceID: serviceID,//前台服务ID
                    templateID: params.serviceId,//引擎模板服务ID
                    taskId: params.taskId,//引擎模板服务taskID
                    taskName: "全国税务事务所查询",//服务名称
                    params: {//请求参数
                        articleField02: req.body.articleField02,
                        articleField03: req.body.articleField03,
                        articleField07: req.body.articleField07,
                        articleField09: req.body.articleField09,
                        cPage: req.body.cPage || '1',//页码
                        randCode: req.body.randCode,
                        scount: req.body.scount
                    },
                    results: JSON.stringify(results)//返回结果
                };
                userLogDAO.add(logData);
            }).catch((err) => {
            res.send(err);
        });
    }
};


const getPicCode = (req, res, next) => {
    const params = {
        sessionId: req.body.randomKey,
        taskId: "3",
        serviceId: "taxFirms",
        userID: req.body.userID,//用户userID
        clientID: req.body.clientID,//用户设备id
        cityID: req.body.cityID,//城市代号
        local_x: req.body.local_x,//城市经度
        local_y: req.body.local_y//城市纬度
    };
    if (util.isEmptyValue("taxOfficeCon.getPicCode", params)) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}});
    } else {
        taxOfficeService.taxOffice(params)
            .then((results) => {
                if (results.retCode == "000000") {
                    res.send(results.responseBody.data);
                } else {
                    res.send("/images/refreshCode.png");
                }
            }).catch((err) => {
            res.send("/images/refreshCode.png");
        });
    }
}

const checkVerification = (req, res, next) => {
    const params = {
        sessionId: req.body.randomKey,
        taskId: "4",
        serviceId: "taxFirms",
        userID: req.body.userID,//用户userID
        clientID: req.body.clientID,//用户设备id
        cityID: req.body.cityID,//城市代号
        local_x: req.body.local_x,//城市经度
        local_y: req.body.local_y,//城市纬度
        verifyCode: req.body.verifyCode
    };
    if (util.isEmptyValue("taxOfficeCon.checkVerification", params)) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}});
    } else {
        taxOfficeService.taxOffice(params)
            .then((results) => {
                res.send(results);
            }).catch((err) => {
            res.send("/images/refreshCode.png");
        });
    }
}
exports.taxOfficeCon = taxOfficeCon;