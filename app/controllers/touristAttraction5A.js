const urlParse = require('url'),
    ...
    touristAttractionService = require("../logics/touristAttraction5A").touristAttractionService;
const path = "touristAttraction5A/", serviceID = 'GAB0001';
let touristAttractionCon = {};

touristAttractionCon.getCon = (req, res, next) => {
...
const pageCon = (req, res, next) => {
    ...
    console.log("5A旅游景区查询renderName: " + renderName);
...
const apiCon = (req, res, next) => {
    ...
    console.log("5A旅游景区查询请求方法名: " + pathName);
    if (!pathName) return;
    eval(pathName + "(req, res, next)");
};
//5A旅游景区查询所查询
const touristAttractionSearch = (req, res, next) => {
    const params = {
        sessionId: req.body.randomKey,//用户randomKey,用来识别用户的唯一性
        taskId: "1",
        serviceId: "ScenicArea",
    ...
    if (util.isEmptyValue("touristAttractionCon.touristAttractionSearch", params)) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}});
    } else {
        params.searchword2 = req.body.searchword2;//
        params.AADDRESS = req.body.AADDRESS;//
        params.AYEAR = req.body.AYEAR;//
        params.page = req.body.page || 1;
        touristAttractionService.touristAttraction(params)
            .then((results) => {
                res.send(results);
                var logData = {
                    ...
                    taskName: "5A旅游景区查询所查询",//服务名称
                    params: {//请求参数
                        searchword2: req.body.searchword2,//
...
exports.touristAttractionCon = touristAttractionCon;