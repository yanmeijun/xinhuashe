const urlParse = require('url'),
    ...
    exitChinaNationService = require("../logics/exitChinaNation").exitChinaNationService;
const path = "exitChinaNation/";
const serviceID = "TCGJ0001", templateID = "exitChinaNation";
let exitChinaNationCon = {};
exitChinaNationCon.getCon = (req, res, next) => {
...
const pageCon = (req, res, next) => {
    ...
    console.log("退出中国国籍审批初审renderName: " + renderName);
...
const apiCon = (req, res, next) => {
    ...
    console.log("退出中国国籍审批初审方法名: " + pathName);
    if (!pathName) return;
...
const verification = (req, res, next) => {
    ...
    if (util.isEmptyValue("exitChinaNationCon.verification", params)) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}});
    } else {
        exitChinaNationService.postData(params)
            .then((results) => {
...
const orderTime = (req, res, next) => {
    ...
    if (util.isEmptyValue("exitChinaNationCon.orderTime", params)) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}})
    } else {
        exitChinaNationService.postData(params)
            .then((results) => {
...
const confirmOrder = (req, res, next) => {
    ...
    if (util.isEmptyValue("exitChinaNationCon.confirmOrder", params)) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}})
    } else {
        exitChinaNationService.postData(params)
            .then((results) => {
...
const orderAddress = (req, res, next) => {
    ...
    if (util.isEmptyValue("exitChinaNationCon.orderAddress", params)) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}})
    } else {
        exitChinaNationService.postData(params)
            .then((results) => {
...
const reportSearch = (req, res, next) => {
    ...
    console.log(params)
    if (util.isEmptyValue("exitChinaNationCon.reportSearch", params)) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}});
    } else {
        exitChinaNationService.postData(params)
            .then((results) => {
...
exports.exitChinaNationCon = exitChinaNationCon;