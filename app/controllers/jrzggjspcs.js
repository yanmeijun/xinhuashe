const urlParse = require('url'),
    ...
    jrzggjspcsService = require("../logics/jrzggjspcs").jrzggjspcsService;
const path = "jrzggjspcs/";
const serviceID = "JRGJ0001", templateID = "joinChinaNation";
let jrzggjspcsCon = {};
jrzggjspcsCon.getCon = (req, res, next) => {
...
const pageCon = (req, res, next) => {
    ...
    console.log("加入中国国籍审批初审renderName: " + renderName);
...
const apiCon = (req, res, next) => {
    ...
    console.log("加入中国国籍审批初审方法名: " + pathName);
    if (!pathName) return;
...
const verification = (req, res, next) => {
    ...
    if (util.isEmptyValue("jrzggjspcsCon.verification", params)) {
...
const orderTime = (req, res, next) => {
    ...
    if (util.isEmptyValue("jrzggjspcsCon.orderTime", params)) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}})
    } else {
        jrzggjspcsService.postData(params)
            .then((results) => {
...
const confirmOrder = (req, res, next) => {
    ...
    if (util.isEmptyValue("jrzggjspcsCon.confirmOrder", params)) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}})
    } else {
        jrzggjspcsService.postData(params)
            .then((results) => {
...
const orderAddress = (req, res, next) => {
    ...
    if (util.isEmptyValue("jrzggjspcsCon.orderAddress", params)) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}})
    } else {
        jrzggjspcsService.postData(params)
            .then((results) => {
...
const reportSearch = (req, res, next) => {
    ...
    console.log(params)
    if (util.isEmptyValue("jrzggjspcsCon.reportSearch", params)) {
        res.send({"rtnCode": "000000", "rtnMsg": "success", "data": {"detail": "请求参数不完整，请检查！"}});
    } else {
        jrzggjspcsService.postData(params)
            .then((results) => {
...
exports.jrzggjspcsCon = jrzggjspcsCon;